...
(function() {
    ...
    var outputs = {
        shape: {
            ...
            event: function() {
                ...
                var min = Math.PI * parseFloat(mod.min.value) / 180
                var max = Math.PI * parseFloat(mod.max.value) / 180
                var radius = parseFloat(mod.radius.value)
                var fn = `((${radius})*(${radius})-((X-(${x}))*(X-(${x}))+(Y-(${y}))*(Y-(${y}))))`
                var fn = `Math.min(${fn},Math.atan2(Y-(${y}),X-(${x}))-(${min}))`
                var fn = `Math.min(${fn},(${max})-Math.atan2(Y-(${y}),X-(${x})))`
                var variables = ['X', 'Y']
                var limits = [
                    [x - radius, x + radius],
                    [y - radius, y + radius]
    ...
    var interface = function(div) {
        ...
        // angle
        //
        div.appendChild(document.createTextNode('angle (-180:180)'))
        div.appendChild(document.createElement('br'))
